...
- 撤掉「升级低质量」整条功能：自动升级开关、低质量统计、单集低质量标签、升级按钮全部移除
- 恢复为干净的功能集：检测已有文件、批量下载、去重、操作日志

## 0.4.8

- 修复：源端实际没有更高码率时（如喜马拉雅 RSS 虚报 189MB、实际只给 48MB），升级判定为「无更高码率」跳过而非失败
- 记住「该源无更高码率」的结论，不再反复标记低质量、不再重复尝试升级

## 0.4.7

- 新增操作日志：记录下载/升级/扫描/去重事件，保留最近 500 条，重启不丢
- 新增每档「升级低质量」按钮（只升级当前节目）
- 节目页显示「低质量 N 集」，单集标「低质量」标签
- 扫描完成消息区分「新下 / 升级 / 已存在 / 失败」
